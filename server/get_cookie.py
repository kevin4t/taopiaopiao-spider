...
class UsernameLogin:
    ...
    def _verify_password(self):
        ...
        verify_password_data = {
            ...
            '_tb_token': 'e3eb533577e36',
            'action': 'LoginAction',
            'event_submit_do_login': 1,
            'loginFrom': 'WAP_TAOBAO',
            'otherLoginUrl': 'https://login.m.taobao.com/msg_login.htm',
            'page': 'loginV3',
            'ncoToken': '94ddff89fec4770668bcc8061c69946a5ae2746c',
            'slideCodeShow': 'false',
            'useMobile': 'false',
            'lang': 'zh_CN',
            'loginsite': 0,
            ...
            'showAssistantLink': '',
            'um_token': 'T19A657F26D39655183A257804ADD1C2B433E4A6EC8319B7FEA13B889BB',
            'ua': self.ua
        }
        try:
        ...
        # 提取申请st码url
        apply_st_url_match = re.search(
            r'<script src="(.*?)"></script>', response.text)
        # 存在则返回
    ...
    def login(self):
        ...
        my_taobao_match = re.search(
            r'top.location.href = "(.*?)"', response.text)
        if my_taobao_match:
    ...
    def get_taobao_nick_name(self):
        ...
        nick_name_match = re.search(
            r'<input id="mtb-nickname" type="hidden" value="(.*?)"/>', response.text)
        if nick_name_match:
...
def get_init_cookie():
    ...
    ua = ua = '121#'
    # 加密后的密码，从浏览器或抓包工具中复制，可重复使用
